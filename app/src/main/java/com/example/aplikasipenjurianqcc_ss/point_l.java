...
public class point_l extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_point_l);
        auto_1 = (AutoCompleteTextView)findViewById(R.id.isi_spinner_l1);
        auto_2 = (AutoCompleteTextView)findViewById(R.id.isi_spinner_l2);
        ...
        myRef1.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                data1 = dataSnapshot.child("l1").getValue(String.class);
                data2 = dataSnapshot.child("l2").getValue(String.class);
            ...
            public void onCancelled(DatabaseError error) {
                Toast.makeText(point_l.this, "Error!", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void input_database(View view) {
        if(auto_1.getText().toString().equals("") || auto_2.getText().toString().equals("")){
            Toast.makeText(point_l.this, "Mohon lengkapi data terlebih dahulu!", Toast.LENGTH_SHORT).show();
        }else{
            FirebaseDatabase database = FirebaseDatabase.getInstance();
            ...
            if(data1.equals("")){
                myRef.child("l1").setValue(login_page.nip_juri +":"+auto_1.getText().toString());
            }else{
                if(data1.contains(login_page.nip_juri)){
                    myRef.child("l1").setValue(data1.replace(data1_split,login_page.nip_juri +":"+auto_1.getText().toString()));
                }else{
                    myRef.child("l1").setValue(data1 +","+ login_page.nip_juri +":"+auto_1.getText().toString());
                }
            }
            if(data2.equals("")){
                myRef.child("l2").setValue(login_page.nip_juri +":"+auto_2.getText().toString());
            }else{
                if(data2.contains(login_page.nip_juri)){
                    myRef.child("l2").setValue(data2.replace(data2_split,login_page.nip_juri +":"+auto_2.getText().toString()));
                }else{
                    myRef.child("l2").setValue(data2 +","+ login_page.nip_juri +":"+auto_2.getText().toString());
                }
            }

            Toast.makeText(point_l.this, "Nilai Dimasukkan", Toast.LENGTH_SHORT).show();
            startActivity(new Intent(point_l.this,list_item_penilaian.class));
        }
    }
}